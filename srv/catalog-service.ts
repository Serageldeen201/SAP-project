...
import { fetchExternalRatingByCategory } from './external/fakestore-client';

const LOG = cds.log('catalog-service');
const { SELECT, INSERT, UPDATE } = cds.ql;

class CatalogService extends (cds.ApplicationService as any) {
    async init(): Promise<void> {
        ...
        this.before(['CREATE', 'UPDATE'], Products, async (req: any) => {
        ...
        this.before(['CREATE', 'UPDATE'], ProductReviews, async (req: any) => {
            const { rating } = req.data;
            if (rating !== undefined && rating !== null) {
                const numericRating = Number(rating);
                if (!Number.isInteger(numericRating) || numericRating < 1 || numericRating > 5) {
        ...
        this.before('CREATE', Products, async (req: any) => {
            ...
            if (category && externalRating === undefined) {
                try {
                    ...
                    const fetchedRating = await fetchExternalRatingByCategory(category);
        ...
        this.on('submitReview', async (req: any) => {
            ...
            const newReviewId = cds.utils.uuid();
